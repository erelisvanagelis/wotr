...
func can_army_enter(another_army: Army) -> bool:
	if entry_conditions == null:
		entry_conditions = CompositeCondition.all(
			"Movement coditions to be met",
			[
				SingleCondition.new("Not attacking another army", func() -> bool: return !army || army.faction == another_army.faction),
				SingleCondition.new("Number of army units <= 10 after the merge", func() -> bool: return can_units_fit(another_army.get_selected_units())),
				CompositeCondition.any(
					"Any of these conditions met",
					[
						SingleCondition.new("Units belong to the target region nation", func() -> bool: return are_units_from_the_same_nation(another_army.units, nation)),
						SingleCondition.new("Units are at war", func() -> bool: return are_units_at_war(another_army.units)),
