...
@export var army: Army:
	set(new_army):
		if army && army.selected.is_connected(on_army_selected):
			army.selected.disconnect(on_army_selected)
		...
		if !army.selected.is_connected(on_army_selected):
			army.selected.connect(on_army_selected)
...
func can_army_enter(another_army: Army) -> bool:
	if !can_units_fit(another_army.get_selected_units()):
		return false

	if nation == free_regions:
		return true
	elif are_units_at_war(another_army.units):
		return true
	elif are_units_from_the_same_nation(another_army.units, nation):
		return true
...
func can_units_fit(units: Array[Unit]) -> bool:
	...
	return units.size() + army.units.size() <= 3
