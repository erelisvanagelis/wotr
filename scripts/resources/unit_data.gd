...
@export var unit_card: PackedScene = preload("res://scenes/ui/unit_card/unit_card.tscn")
...
static func select_unit(unit_nation: Nation, unit_type: String) -> UnitData:
	...
	if unit_nation.faction.title == "Free People":
		return load("res://scripts/resources/units/free_unit.tres")

	return load("res://scripts/resources/units/shadow_unit.tres")
